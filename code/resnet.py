...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        if not args.depthLayers:
            out = F.avg_pool2d(out, 4)
            out = out.view(out.size(0), -1)
            out = self.linear(out)
        else:
            # sparse to dense
            out = self.layer_convEnc(out)
            out = self.layer_batchEnc(out)
            out = self.layer_upsampDec1(out)
            out = self.layer_upsampDec2(out)
            out = self.layer_upsampDec3(out)
            out = self.layer_upsampDec4(out)
            out = self.layer_convDec(out)
            out = self.layer_upsampDec5(out)
        return out
